...
const getSSOAppTargetLocation = (
    location: TargetLocation = window.location
): TargetLocation => {
...
export const getAppHref = (
    ...
    const secondLevelDomain =
        lastIndex !== secondLevelIndex
            ? hostname.substr(secondLevelIndex + 1)
            : hostname;
    const targetDomain = [targetSubdomain, secondLevelDomain]
        .filter(isTruthy)
        .join(".");
