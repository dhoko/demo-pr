...
const refresh = (call, UID, attempts, maxAttempts) => {
    return call(withUIDHeaders(UID, setRefreshCookies())).catch((e) => {
        ...
        if (name === "OfflineError") {
            ...
            return wait(OFFLINE_RETRY_DELAY).then(() =>
                refresh(call, UID, attempts + 1, OFFLINE_RETRY_ATTEMPTS_MAX)
            );
        }
        ...
        if (status === HTTP_ERROR_CODES.TOO_MANY_REQUESTS) {
            return retryHandler(e).then(() =>
                refresh(call, UID, attempts + 1, RETRY_ATTEMPTS_MAX)
            );
        }
                ...
                async (responseDate = new Date()) => {
                    ...
                    try {
                        ...
                        if (
                            ...
                            setLastRefreshDate(
                                UID,
                                getDateHeader(result.headers) || new Date()
                            );
        ...
        const perform = (attempts, maxAttempts) => {
            ...
            return call(options).catch((e) => {
                ...
                if (name === "OfflineError") {
                    ...
                    return wait(OFFLINE_RETRY_DELAY).then(() =>
                        perform(attempts + 1, retriesOnOffline)
                    );
                }
                ...
                if (
                    ...
                    !ignoreUnauthorized &&
                    (UID ||
                        (requestUID && !(headers && !!headers.Authorization)))
                ) {
                    return refreshHandler(
                        requestUID,
                        getDateHeader(response && response.headers)
                    ).then(
                ...
                const ignoreUnlock =
                    Array.isArray(ignoreHandler) &&
                    ignoreHandler.includes(HTTP_ERROR_CODES.UNLOCK);
                ...
                const ignoreTooManyRequests =
                    Array.isArray(ignoreHandler) &&
                    ignoreHandler.includes(HTTP_ERROR_CODES.TOO_MANY_REQUESTS);
                if (
                    status === HTTP_ERROR_CODES.TOO_MANY_REQUESTS &&
                    !ignoreTooManyRequests
                ) {
                    return retryHandler(e).then(
                        () => perform(attempts + 1, RETRY_ATTEMPTS_MAX),
                        () => onError(e)
