...
import {
    getDateFnLocaleWithLongFormat,
    getDateFnLocaleWithSettings,
    Options,
} from "./dateFnLocale";
...
export const loadDateLocale = async (
    ...
    const closestLocaleCode =
        getClosestLocaleMatch(localeCode, dateFnLocales) || DEFAULT_LOCALE;
    const closestBrowserLocaleCode =
        getClosestLocaleMatch(browserLocaleCode, dateFnLocales) ||
        DEFAULT_LOCALE;
    ...
    const mergedDateLocale = getDateFnLocaleWithLongFormat(
        dateFnLocale,
        browserDateFnLocale
    );
    const updatedDateFnLocale = getDateFnLocaleWithSettings(
        mergedDateLocale,
        options
    );
