import {
    queryAddresses,
    setupAddress as setupAddressRoute,
} from "../api/addresses";
...
export const handleSetupAddress = async ({
    api,
    username,
    domains,
}: SetupAddressArgs) => {
...
export const handleSetupAddressKeys = async ({
    ...
    const [availableAddresses, availableDomains] = await Promise.all([
        api<{ Addresses: Address[] }>(queryAddresses()).then(
            ({ Addresses }) => Addresses
        ),
        api<{ Domains: string[] }>(queryAvailableDomains()).then(
            ({ Domains }) => Domains
        ),
    ]);
