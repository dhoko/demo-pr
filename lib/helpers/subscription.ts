...
export const getPlan = (
    ...
    return Plans.find(
        ({ Services, Type }) => Type === PLAN && Services & service
    );
};

export const getPlans = ({ Plans = [] }: Subscription) =>
    Plans.filter(({ Type }) => Type === PLAN);
export const getAddons = ({ Plans = [] }: Subscription) =>
    Plans.filter(({ Type }) => Type === ADDON);
export const hasAddons = ({ Plans = [] }: Subscription) =>
    Plans.some(({ Type }) => Type === ADDON);
...
export const getMonthlyBaseAmount = (
    ...
    return subscription.Plans.filter(({ Name }) => Name === name).reduce(
        (acc) => acc + base.Pricing[CYCLE.MONTHLY],
        0
    );
};
