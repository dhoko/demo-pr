...
export const getIsRruleSimple = (rrule?: VcalRrulePropertyValue): boolean => {
    ...
    const hasUnsupportedFields = nonEmptyFields.some(
        (field) => !SUPPORTED_RRULE_PROPERTIES.includes(field)
    );
    ...
    if (freq === FREQUENCY.DAILY) {
        if (
            nonEmptyFields.some(
                (field) => !SUPPORTED_RRULE_PROPERTIES_DAILY.includes(field)
            )
        ) {
            return false;
        }
    ...
    if (freq === FREQUENCY.WEEKLY) {
        if (
            nonEmptyFields.some(
                (field) => !SUPPORTED_RRULE_PROPERTIES_WEEKLY.includes(field)
            )
        ) {
            return false;
        }
    ...
    if (freq === FREQUENCY.MONTHLY) {
        if (
            nonEmptyFields.some(
                (field) => !SUPPORTED_RRULE_PROPERTIES_MONTHLY.includes(field)
            )
        ) {
            return false;
        }
    ...
    if (freq === FREQUENCY.YEARLY) {
        if (
            nonEmptyFields.some(
                (field) => !SUPPORTED_RRULE_PROPERTIES_YEARLY.includes(field)
            )
        ) {
            return false;
        }
...
export const getIsRruleCustom = (rrule?: VcalRrulePropertyValue): boolean => {
    ...
    const isBasicCustom =
        (interval && interval > 1) || (count && count >= 1) || !!until;
    if (freq === FREQUENCY.DAILY) {
        if (
            nonEmptyFields.some(
                (field) => !SUPPORTED_RRULE_PROPERTIES_DAILY.includes(field)
    ...
    if (freq === FREQUENCY.MONTHLY) {
        ...
        if (
            isLongArray(byday) ||
            isLongArray(bymonthday) ||
            isLongArray(bysetpos)
        ) {
            return false;
        }
...
export const getIsRruleSupported = (
    rruleProperty?: VcalRrulePropertyValue,
    isInvitation = false
) => {
    if (!rruleProperty) {
        return false;
    }
    const rruleProperties = Object.keys(
        rruleProperty
    ) as (keyof VcalRrulePropertyValue)[];
    const hasUnsupportedProperties = isInvitation
        ? rruleProperties.some(
              (property) =>
                  !SUPPORTED_RRULE_PROPERTIES_INVITATION.includes(property)
          )
        : rruleProperties.some(
              (property) => !SUPPORTED_RRULE_PROPERTIES.includes(property)
          );
    ...
    if (freq === "DAILY") {
        ...
        if (
            rruleProperties.some(
                (property) =>
                    !SUPPORTED_RRULE_PROPERTIES_DAILY.includes(property)
            )
        ) {
            return false;
        }
    ...
    if (freq === "WEEKLY") {
        ...
        if (
            rruleProperties.some(
                (property) =>
                    !SUPPORTED_RRULE_PROPERTIES_WEEKLY.includes(property)
            )
        ) {
            return false;
        }
    ...
    if (freq === "MONTHLY") {
        ...
        if (
            rruleProperties.some(
                (property) =>
                    !SUPPORTED_RRULE_PROPERTIES_MONTHLY.includes(property)
            )
        ) {
            return false;
        }
        ...
        if (!!day && !!setpos) {
            return (
                getIsStandardByday(day) &&
                getIsSupportedSetpos(setpos) &&
                !bymonthday
            );
        }
        if (+!!day ^ +!!setpos) {
    ...
    if (freq === "YEARLY") {
        ...
        if (
            rruleProperties.some(
                (property) =>
                    !SUPPORTED_RRULE_PROPERTIES_YEARLY.includes(property)
            )
        ) {
            return false;
        }
