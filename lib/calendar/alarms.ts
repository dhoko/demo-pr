...
export const getAlarmMessage = ({
    ...
    const startFakeUTCDate = toUTCDate(
        convertUTCDateTimeToZone(fromUTCDate(utcStartDate), tzid)
    );
    const nowFakeUTCDate = toUTCDate(
        convertUTCDateTimeToZone(fromUTCDate(now), tzid)
    );
...
const isAbsoluteTrigger = (
    trigger: VcalTriggerProperty
): trigger is VcalDateTimeProperty => {
    return (trigger as VcalDateTimeProperty).parameters?.type === "date-time";
};

const absoluteToRelative = (
    trigger: VcalDateTimeProperty,
    ...
    return {
        ...
        minutes,
        seconds: 0,
        isNegative: durationInMinutes >= 0,
    };
};
 ...
 */
const normalizeDurationToUnit = (
    duration: Partial<VcalDurationValue>,
    unit: number
) => {
...
export const normalizeTrigger = (
    ...
    const duration = isAbsoluteTrigger(trigger)
        ? absoluteToRelative(trigger, dtstart)
        : trigger.value;
    ...
    if (getIsPropertyAllDay(dtstart)) {
        ...
        const mustNormalize = duration.isNegative
            ? weeks > 0 && days !== 6
            : weeks > 0 && days !== 0;
    ...
    if (duration.minutes % 60 !== 0) {
        return {
            ...result,
            minutes: normalizeDurationToUnit(duration, MINUTE),
        };
    }
...
export const getIsValidAlarm = (alarm: VcalValarmComponent) => {
    ...
    if (
        isAbsoluteTrigger(trigger) &&
        !getIsDateTimeValue(trigger.value as DateTimeValue)
    ) {
        return false;
    }
...
export const getSupportedAlarm = (
    ...
    if (
        !isAbsoluteTrigger(trigger) &&
        trigger.parameters?.related?.toLocaleLowerCase() === "end"
    ) {
        return;
    }
    ...
    const triggerDurationInSeconds = normalizeDurationToUnit(
        normalizedTrigger,
        1
    );
    ...
    const nonSupportedTrigger =
        ...
        normalizedTrigger.minutes >
            NOTIFICATION_UNITS_MAX[NOTIFICATION_UNITS.MINUTES] ||
        normalizedTrigger.hours >
            NOTIFICATION_UNITS_MAX[NOTIFICATION_UNITS.HOURS] ||
        normalizedTrigger.days >
            NOTIFICATION_UNITS_MAX[NOTIFICATION_UNITS.DAY] ||
        normalizedTrigger.weeks >
            NOTIFICATION_UNITS_MAX[NOTIFICATION_UNITS.WEEK];
 ...
 */
export const filterFutureNotifications = (
 ...
 */
export const filterEmailNotifications = (
    notifications: NotificationModel[]
) => {
...
export const sortNotificationsByAscendingTrigger = (
    notifications: NotificationModel[]
) =>
    [...notifications].sort((a: NotificationModel, b: NotificationModel) => {
        ...
        const triggerAMinutes =
            normalizeDurationToUnit(triggerA, NOTIFICATION_UNITS.MINUTES) *
            (triggerA.isNegative ? -1 : 1);
        const triggerBMinutes =
            normalizeDurationToUnit(triggerB, NOTIFICATION_UNITS.MINUTES) *
            (triggerB.isNegative ? -1 : 1);

        return triggerAMinutes - triggerBMinutes;
    });

const sortNotificationsByAscendingValue = (
    a: NotificationModel,
    b: NotificationModel
) => (a.value || 0) - (b.value || 0);
...
const uniqueNotificationComparator = (notification: NotificationModel) => {
    ...
    return `${notification.type}-${
        normalizeDurationToUnit(trigger, NOTIFICATION_UNITS.MINUTES) *
...
export const dedupeNotifications = (notifications: NotificationModel[]) => {
    const sortedNotifications = [...notifications].sort(
        sortNotificationsByAscendingValue
    );

    return uniqueBy(sortedNotifications, uniqueNotificationComparator);
};

const getSmallestNonZeroNumericValueFromDurationValue = (
    object: VcalDurationValue
) => Math.min(...Object.values(omit(object, ["isNegative"])).filter(Boolean));

const sortAlarmsByAscendingTriggerValue = (
    a: VcalValarmRelativeComponent,
    b: VcalValarmRelativeComponent
) => {
    const aMin = getSmallestNonZeroNumericValueFromDurationValue(
        a.trigger.value
    );
    const bMin = getSmallestNonZeroNumericValueFromDurationValue(
        b.trigger.value
    );
...
const uniqueAlarmComparator = (alarm: VcalValarmRelativeComponent) => {
    ...
    const isTriggerNegative =
        "isNegative" in triggerValue && triggerValue.isNegative;

    return `${alarm.action.value}-${
        normalizeDurationToUnit(triggerValue, NOTIFICATION_UNITS.MINUTES) *
        (isTriggerNegative ? -1 : 1)
    }`;
};
 ...
 */
export const dedupeAlarmsWithNormalizedTriggers = (
    alarms: VcalValarmRelativeComponent[]
) => {
