...
export const getIsCalendarDisabled = ({ Flags } = { Flags: 0 }) => {
    return (
        hasBit(Flags, CALENDAR_FLAGS.SELF_DISABLED) ||
        hasBit(Flags, CALENDAR_FLAGS.SUPER_OWNER_DISABLED)
    );
};
...
export const getIsCalendarProbablyActive = (calendar = { Flags: 0 }) => {
    ...
    return (
        calendar.Flags === undefined ||
        (!getIsCalendarDisabled(calendar) && getIsCalendarActive(calendar))
    );
};
