...
const getAlarmMessageText = ({
    ...
    if (!isInFuture) {
        if (isSameDay(nowFakeUTCDate, startFakeUTCDate)) {
            ...
            return c("Alarm notification")
                .t`${title} started at ${formattedHour}`;
        }
        if (isNextDay(startFakeUTCDate, nowFakeUTCDate)) {
            ...
            return c("Alarm notification")
                .t`${title} started yesterday at ${formattedHour}`;
        }
        ...
        if (isSameMonth(nowFakeUTCDate, startFakeUTCDate)) {
            ...
            if (isAllDay) {
                return c("Alarm notification")
                    .t`${title} started on ${formattedDate}`;
            }
            return c("Alarm notification")
                .t`${title} started on ${formattedDate} at ${formattedHour}`;
        }
        ...
        if (isAllDay) {
            const formattedDateWithoutTime = formatUTC(
                startFakeUTCDate,
                "PPPP",
                formatOptions
            );
            return c("Alarm notification")
                .t`${title} started on ${formattedDateWithoutTime}`;
        }
        const formattedDateWithTime = formatUTC(
            startFakeUTCDate,
            "PPPPp",
            formatOptions
        );
        return c("Alarm notification")
            .t`${title} started on ${formattedDateWithTime}`;
    }
    ...
    if (isSameDay(nowFakeUTCDate, startFakeUTCDate)) {
        ...
        return c("Alarm notification")
            .t`${title} will start at ${formattedHour}`;
    }
    ...
    if (isNextDay(nowFakeUTCDate, startFakeUTCDate)) {
        ...
        return c("Alarm notification")
            .t`${title} will start tomorrow at ${formattedHour}`;
    }
    ...
    if (isSameMonth(nowFakeUTCDate, startFakeUTCDate)) {
        ...
        if (isAllDay) {
            return c("Alarm notification")
                .t`${title} will start on ${formattedDate}`;
        }
        return c("Alarm notification")
            .t`${title} will start on ${formattedDate} at ${formattedHour}`;
    }
    ...
    if (isAllDay) {
        ...
        return c("Alarm notification")
            .t`${title} will start on ${formattedDateWithoutTime}`;
    }
    ...
    return c("Alarm notification")
        .t`${title} will start on ${formattedDateWithTime}`;
};
