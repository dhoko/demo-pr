...
const getMemberAndAddress = (
    ...
    const selfMember = selfAddress
        ? Members.find((Member) => Member.Email === selfAddress.Email)
        : undefined;
    ...
    const Address = Addresses.find(
        ({ Email }) => defaultMember.Email === Email
    );
    if (!Address) {
...
export const getMemberAndAddressID = ([{ ID: memberID }, { ID: addressID }]: [
    MemberInterface,
    AddressInterface
]) => [memberID, addressID];
