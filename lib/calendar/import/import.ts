...
const getParsedComponentHasError = (
    component: VcalCalendarComponentOrError
): component is { error: Error } => {
    ...
    try {
        ...
        if (!getIsCalendar(parsedVcalendar)) {
            throw new ImportFileError(
                IMPORT_ERROR_TYPE.INVALID_CALENDAR,
                filename
            );
        }
        const {
            ...
            components,
            calscale,
            "x-wr-timezone": xWrTimezone,
        } = parsedVcalendar;
        if (version?.value !== "2.0") {
            throw new ImportFileError(
                IMPORT_ERROR_TYPE.INVALID_VERSION,
                filename
            );
        }
        if (method && method.value.toLowerCase() !== "publish") {
            throw new ImportFileError(
                IMPORT_ERROR_TYPE.INVALID_METHOD,
                filename
            );
        }
        ...
        if (components.length > MAX_IMPORT_EVENTS) {
            throw new ImportFileError(
                IMPORT_ERROR_TYPE.TOO_MANY_EVENTS,
                filename
            );
        }
        return {
...
const getComponentIdentifier = (
    ...
    if (getIsEventComponent(vcalComponent)) {
        ...
        if (dtstart?.value) {
            return formatUTC(toUTCDate(dtstart.value), "PPpp", {
                locale: dateLocale,
            });
        }
...
const extractGuessTzid = (components: VcalCalendarComponentOrError[]) => {
    const vtimezone = components.find(
        (componentOrError): componentOrError is VcalVtimezoneComponent => {
...
export const getSupportedDateOrDateTimeProperty = ({
    ...
    if (
        partDayProperty.value.isUTC &&
        !isRecurring &&
        hasXWrTimezone &&
        calendarTzid
    ) {
        const localizedDateTime = convertUTCDateTimeToZone(
            partDayProperty.value,
            calendarTzid
        );
    ...
    if (!partDayPropertyTzid) {
        if (!hasXWrTimezone) {
            throw new ImportEventError(
                IMPORT_EVENT_ERROR_TYPE.FLOATING_TIME,
                "vevent",
                componentId
            );
        }
        if (hasXWrTimezone && !calendarTzid) {
            throw new ImportEventError(
                IMPORT_EVENT_ERROR_TYPE.X_WR_TIMEZONE_UNSUPPORTED,
                "vevent",
                componentId
            );
        }
    ...
    if (!supportedTzid) {
        throw new ImportEventError(
            IMPORT_EVENT_ERROR_TYPE.TZID_UNSUPPORTED,
            component,
            componentId
        );
    }
...
const getLinkedDateTimeProperty = ({
    ...
    if (getIsPropertyAllDay(property)) {
        throw new ImportEventError(
            IMPORT_EVENT_ERROR_TYPE.ALLDAY_INCONSISTENCY,
            component,
            componentId
        );
    }
...
export const getSupportedEvent = ({
    ...
    if (getIsTodoComponent(vcalComponent)) {
        throw new ImportEventError(
            IMPORT_EVENT_ERROR_TYPE.TODO_FORMAT,
            "vtodo",
            componentId
        );
    }
    if (getIsJournalComponent(vcalComponent)) {
        throw new ImportEventError(
            IMPORT_EVENT_ERROR_TYPE.JOURNAL_FORMAT,
            "vjournal",
            componentId
        );
    }
    if (getIsFreebusyComponent(vcalComponent)) {
        throw new ImportEventError(
            IMPORT_EVENT_ERROR_TYPE.FREEBUSY_FORMAT,
            "vfreebusy",
            componentId
        );
    }
    if (getIsTimezoneComponent(vcalComponent)) {
        if (!getSupportedTimezone(vcalComponent.tzid.value)) {
            throw new ImportEventError(
                IMPORT_EVENT_ERROR_TYPE.TIMEZONE_FORMAT,
                "vtimezone",
                componentId
            );
        }
        throw new ImportEventError(
            IMPORT_EVENT_ERROR_TYPE.TIMEZONE_IGNORE,
            "vtimezone",
            componentId
        );
    }
    if (!getIsEventComponent(vcalComponent)) {
        throw new ImportEventError(
            IMPORT_EVENT_ERROR_TYPE.WRONG_FORMAT,
            "vunknown",
            componentId
        );
    }
    if (!getHasUid(vcalComponent)) {
        throw new ImportEventError(
            IMPORT_EVENT_ERROR_TYPE.UID_MISSING,
            "vevent",
            componentId
        );
    }
    if (!getHasDtStart(vcalComponent)) {
        throw new ImportEventError(
            IMPORT_EVENT_ERROR_TYPE.DTSTART_MISSING,
            "vevent",
            componentId
        );
    }
    try {
        ...
        const validated: VcalVeventComponent &
            Required<
                Pick<VcalVeventComponent, "uid" | "dtstamp" | "dtstart">
            > = {
            component,
        ...
        if (trimmedDescriptionValue) {
            validated.description = {
                ...description,
                value: truncate(
                    trimmedDescriptionValue,
                    MAX_LENGTHS.EVENT_DESCRIPTION
                ),
            };
        }
        ...
        const sequenceSafeValue = Number.isSafeInteger(sequenceValue)
            ? sequenceValue
            : 0;
        ...
        if (!getIsWellFormedDateOrDateTime(validated.dtstart)) {
            throw new ImportEventError(
                IMPORT_EVENT_ERROR_TYPE.DTSTART_MALFORMED,
                "vevent",
                componentId
            );
        }
        if (getIsDateOutOfBounds(validated.dtstart)) {
            throw new ImportEventError(
                IMPORT_EVENT_ERROR_TYPE.DTSTART_OUT_OF_BOUNDS,
                "vevent",
                componentId
            );
        }
        if (dtend) {
            ...
            if (!getIsWellFormedDateOrDateTime(supportedDtend)) {
                throw new ImportEventError(
                    IMPORT_EVENT_ERROR_TYPE.DTEND_MALFORMED,
                    "vevent",
                    componentId
                );
            }
            if (getIsDateOutOfBounds(supportedDtend)) {
                throw new ImportEventError(
                    IMPORT_EVENT_ERROR_TYPE.DTEND_OUT_OF_BOUNDS,
                    "vevent",
                    componentId
                );
            }
            ...
            const modifiedEndDateUTC =
                !getIsPropertyAllDay(dtend) || +startDateUTC === +endDateUTC
                    ? endDateUTC
                    : addDays(endDateUTC, -1);
            ...
            throw new ImportEventError(
                IMPORT_EVENT_ERROR_TYPE.VEVENT_DURATION,
                "vevent",
                componentId
            );
        }
        const isAllDayEnd = validated.dtend
            ? getIsPropertyAllDay(validated.dtend)
            : undefined;
        ...
        if (exdate) {
            if (!rrule) {
                throw new ImportEventError(
                    IMPORT_EVENT_ERROR_TYPE.RRULE_MALFORMED,
                    "vevent",
                    componentId
                );
            }
        ...
        if (recurrenceId) {
            if (rrule) {
                throw new ImportEventError(
                    IMPORT_EVENT_ERROR_TYPE.SINGLE_EDIT_UNSUPPORTED,
                    "vevent",
                    componentId
                );
            }
        ...
        if (rrule) {
            const supportedRrule = getSupportedRrule(
                { ...validated, rrule },
                false,
                guessTzid
            );
            if (!supportedRrule) {
                throw new ImportEventError(
                    IMPORT_EVENT_ERROR_TYPE.RRULE_UNSUPPORTED,
                    "vevent",
                    componentId
                );
            }
        ...
        const alarms =
            components?.filter(({ component }) => component === "valarm") || [];
        const supportedAlarms = getSupportedAlarms(alarms, dtstart);
        const dedupedAlarms = dedupeAlarmsWithNormalizedTriggers(
            supportedAlarms
        );
        ...
        throw new ImportEventError(
            IMPORT_EVENT_ERROR_TYPE.VALIDATION_ERROR,
            "vevent",
            componentId
        );
    }
...
export const getSupportedEvents = ({
    ...
    const calendarTzid = xWrTimezone
        ? getSupportedTimezone(xWrTimezone)
        : undefined;
    ...
    return components
        .map((vcalComponent) => {
            try {
                ...
                if (
                    e instanceof ImportEventError &&
                    e.type === IMPORT_EVENT_ERROR_TYPE.TIMEZONE_IGNORE
                ) {
                    return;
                }
                return e;
...
export const splitByRecurrenceId = (events: VcalVeventComponent[]) => {
    return events.reduce<{
        ...
        withRecurrenceId: (VcalVeventComponent &
            Required<Pick<VcalVeventComponent, "recurrence-id">>)[];
    }>(
        (acc, event) => {
...
const getParentEventFromApi = async (
    uid: string,
    api: Api,
    calendarId: string
) => {
...
interface GetSupportedEventsWithRecurrenceIdArgs {
    eventsWithRecurrenceId: (VcalVeventComponent &
        Required<Pick<VcalVeventComponent, "recurrence-id">>)[];
...
export const getSupportedEventsWithRecurrenceId = async ({
    ...
    const uidsToFetch = unique(
        eventsWithRecurrenceId
            .filter(({ uid }) => !mapParentEvents[uid.value])
            .map(({ uid }) => uid.value)
    );
    const result = await Promise.all(
        uidsToFetch.map((uid) => getParentEventFromApi(uid, api, calendarId))
    );
    ...
    return eventsWithRecurrenceId.map((event) => {
        ...
        if (!mapParentEvents[uid]) {
            return new ImportEventError(
                IMPORT_EVENT_ERROR_TYPE.PARENT_EVENT_MISSING,
                "vevent",
                uid
            );
        }
