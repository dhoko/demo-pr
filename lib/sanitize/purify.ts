...
const LIST_STYLE_PROPERTIES_REMOVED = [
    "position",
    "left",
    "right",
    "top",
...
const CONFIG: { [key: string]: any } = {
    ...
    protonizer: {
        ...
        ADD_ATTR: [
            "target",
            ...LIST_PROTON_ATTR.map((attr) => `proton-${attr}`),
        ],
        WHOLE_DOCUMENT: true,
        RETURN_DOM: true,
    },
...
const getConfig = (type: string): Config => ({
    ...CONFIG.default,
    ...(CONFIG[type] || {}),
});
...
const sanitizeElements = (document: Element) => {
    LIST_PROTON_TAG.forEach((tagName) => {
        ...
        svgs.forEach((element) => {
            const newElement = element.ownerDocument.createElement(
                `proton-${tagName}`
            );
            // Copy the children
...
const beforeSanitizeElements = (node: Node) => {
    ...
    Array.from(element.attributes).forEach((type) => {
        ...
        if (item === "style") {
            const escaped = escapeURLinStyle(
