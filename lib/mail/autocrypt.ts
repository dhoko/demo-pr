...
export const getParsedAutocryptHeader = (
    header = "",
    sender = ""
): AutocryptHeader | undefined => {
    let invalid = false;
        ...
        header
            ...
            .map((keyValue) => {
                ...
                if (trimmedKeyValue.startsWith("keydata=")) {
                    try {
                        const keydataStringValue = trimmedKeyValue.slice(
                            "keydata=".length
                        );
                        const keydataValue = binaryStringToArray(
                            decodeBase64(keydataStringValue)
                        );
