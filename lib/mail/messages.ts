 ...
 */
export const hasMimeType = (type: MIME_TYPES) => (message?: Partial<Message>) =>
 ...
 */
export const hasFlag = (flag: number) => (message?: Partial<Message>) =>
    hasBit(message?.Flags, flag);
export const setFlag = (flag: number) => (message?: Partial<Message>) =>
    setBit(message?.Flags, flag);
export const clearFlag = (flag: number) => (message?: Partial<Message>) =>
    clearBit(message?.Flags, flag);
export const toggleFlag = (flag: number) => (message?: Partial<Message>) =>
    toggleBit(message?.Flags, flag);
...
export const isOutbox = (message?: Partial<Message>) =>
    message?.LabelIDs?.includes(MAILBOX_LABEL_IDS.OUTBOX);
...
export const isExternalEncrypted = (message: Message) =>
    isE2E(message) && !isInternal(message);
export const isPGPEncrypted = (message: Message) =>
    isExternal(message) && isReceived(message) && isE2E(message);
export const inSigningPeriod = ({ Time = 0 }: Message) =>
    Time >= Math.max(SIGNATURE_START.USER, SIGNATURE_START.BULK);
export const isPGPInline = (message: Message) =>
    isPGPEncrypted(message) && !isMIME(message);
export const isEO = (message?: Partial<Message>) => !!message?.Password;
export const addReceived = (Flags = 0) =>
    setBit(Flags, MESSAGE_FLAGS.FLAG_RECEIVED);
...
export const getParsedHeadersFirstValue = (
    message: Partial<Message> | undefined,
    parameter: string
) => {
...
export const getParsedHeadersAsArray = (
    message: Partial<Message> | undefined,
    parameter: string
) => {
    const value = getParsedHeaders(message, parameter);
...
export const requireReadReceipt = (message?: Message) => {
    const dispositionNotificationTo =
        getParsedHeaders(message, "Disposition-Notification-To") || ""; // ex: Andy <andy@pm.me>

    if (
        !dispositionNotificationTo ||
        isReadReceiptSent(message) ||
        isSent(message)
    ) {
        return false;
    }
...
export const isSentAutoReply = ({ Flags, ParsedHeaders = {} }: Message) => {
    ...
    return (
        ...
        autoReplyHeaderValues.some(([header, searchedValue]) =>
            getParsedHeadersAsArray({ ParsedHeaders }, header)?.some(
                (foundValue) => foundValue === searchedValue
            )
        )
    );
};
